import { $ as e, At as t, Bt as n, Dt as r, Ft as i, It as a, Lt as o, Mt as s, N as c, Pt as l, Q as u, R as d, Rt as f, St as p, Tt as m, Vt as h, Wt as g, X as _, Z as v, bt as y, et as b, ht as x, jt as S, kt as C, pt as w, st as ee, zt as T } from "./dist-DFEez2dM.js";
...
var E = class {
	...
	tokenBefore(e) {
		let t = d(this.state).resolveInner(this.pos, -1);
	...
	matchBefore(e) {
		let t = this.state.doc.lineAt(this.pos), n = Math.max(t.from, this.pos - 250), r = t.text.slice(n - t.from, this.pos - t.from), i = r.search(re(e, !1));
		return i < 0 ? null : {
			from: n + i,
			to: this.pos,
			text: r.slice(i)
...
function O(e) {
	let t = e.map((e) => typeof e == "string" ? { label: e } : e), [n, r] = t.every((e) => /^\w+$/.test(e.label)) ? [/\w*$/, /\w+$/] : te(t);
	return (e) => {
...
function ne(e, t) {
	return (n) => {
		for (let t = d(n.state).resolveInner(n.pos, -1); t; t = t.parent) {
...
function oe(e) {
	...
	let t = ae.get(e);
	return t || ae.set(e, t = O(e)), t;
}
var M = /* @__PURE__ */ i.define(), N = /* @__PURE__ */ i.define(), se = class {
	constructor(e) {
		...
		for (let t = 0; t < e.length;) {
			...
			this.folded.push(T(o == a ? a.toLowerCase() : o, 0)), t += i;
		}
	...
	match(e) {
		...
		if (t.length == 1) {
			let i = T(e, 0), a = n(i), o = a == e.length ? 0 : -100;
			if (i != t[0]) if (i == r[0]) o += -200;
			else return null;
		...
		if (s < 0) {
			for (let a = 0, o = Math.min(e.length, 200); a < o && l < c;) {
				let o = T(e, a);
				(o == t[l] || o == r[l]) && (i[l++] = a), a += n(o);
			}
			if (l < c) return null;
		}
		let u = 0, d = 0, f = !1, p = 0, m = -1, h = -1, _ = /[a-z]/.test(e), v = !0;
		for (let i = 0, l = Math.min(e.length, 200), y = 0; i < l && d < c;) {
			let l = T(e, i);
			s < 0 && (u < c && l == t[u] && (a[u++] = i), p < c && (l == t[p] || l == r[p] ? (p == 0 && (m = i), h = i + 1, p++) : p = 0));
			let b, x = l < 255 ? l >= 48 && l <= 57 || l >= 97 && l <= 122 ? 2 : +(l >= 65 && l <= 90) : (b = g(l)) == b.toLowerCase() ? b == b.toUpperCase() ? 0 : 2 : 1;
			(!i || x == 1 && _ || y == 0 && x != 0) && (t[d] == l || r[d] == l && (f = !0) ? o[d++] = i : o.length && (v = !1)), y = x, i += n(l);
		}
		return d == c && o[0] == 0 && v ? this.result(-100 + (f ? -200 : 0), o, e) : p == c && m == 0 ? this.ret(-200 - e.length + (h == e.length ? 0 : -100), [0, h]) : s > -1 ? this.ret(-700 - e.length, [s, s + this.pattern.length]) : p == c ? this.ret(-900 - e.length, [m, h]) : d == c ? this.result(-100 + (f ? -200 : 0) + -700 + (v ? 0 : -1100), o, e) : t.length == 2 ? null : this.result((i[0] ? -700 : 0) + -200 + -1100, i, e);
	}
	result(e, t, r) {
		let i = [], a = 0;
		for (let e of t) {
			let t = e + (this.astral ? n(T(r, e)) : 1);
	...
	return h(e, {
		...
		closeOnBlur: (e, t) => e && t,
		icons: (e, t) => e && t,
		tooltipClass: (e, t) => (n) => F(e(n), t(n)),
		optionClass: (e, t) => (n) => F(e(n), t(n)),
...
var de = class {
	constructor(e, t, n) {
		...
		let r = e.state.field(t), { options: i, selected: a } = r.open, o = e.state.facet(P);
		this.optionContent = ue(o), this.optionClass = o.optionClass, this.tooltipClass = o.tooltipClass, this.range = L(i.length, a, o.maxRenderedOptions), this.dom = document.createElement("div"), this.dom.className = "cm-tooltip-autocomplete", this.updateTooltipClass(e.state), this.dom.addEventListener("mousedown", (n) => {
			...
			if (n.target == this.list) {
				...
				t != null && (e.dispatch({ effects: I.of(t) }), n.preventDefault());
			}
		}), this.dom.addEventListener("focusout", (t) => {
			let n = e.state.field(this.stateField, !1);
			n && n.tooltip && e.state.facet(P).closeOnBlur && t.relatedTarget != e.contentDOM && e.dispatch({ effects: N.of(null) });
	...
	update(e) {
		...
		if (this.updateTooltipClass(e.state), t != n) {
			...
			(!n.open || n.open.options != r) && (this.range = L(r.length, i, e.state.facet(P).maxRenderedOptions), this.showOptions(r, t.id)), this.updateSel(), a != n.open?.disabled && this.dom.classList.toggle("cm-tooltip-autocomplete-disabled", !!a);
		}
	}
	...
	updateSel() {
		...
		(t.selected > -1 && t.selected < this.range.from || t.selected >= this.range.to) && (this.range = L(t.options.length, t.selected, this.view.state.facet(P).maxRenderedOptions), this.showOptions(t.options, e.id));
	...
	measureInfo() {
		...
		return r.top > Math.min(i.bottom, t.bottom) - 10 || r.bottom < Math.max(i.top, t.top) + 10 ? null : this.view.state.facet(P).positionInfo(this.view, t, r, n, i, this.dom);
	}
	placeInfo(e) {
...
function he(e, t) {
	...
	for (let r of e) if (r.hasResult()) {
		...
		if (r.result.filter === !1) for (let t of r.result.options) a(new k(t, r.source, e ? e(t) : [], 1e9 - n.length));
		else {
			let n = t.sliceDoc(r.from, r.to), s, c = o.filterStrict ? new ce(n) : new se(n);
			for (let t of r.result.options) if (s = c.match(t.label)) {
				let n = t.displayLabel ? e ? e(t, s.matched) : [] : s.matched, o = s.score + (t.boost || 0);
				if (a(new k(t, r.source, n, o)), typeof t.section == "object" && t.section.rank === "dynamic") {
	...
	for (let e of n.sort((e, t) => t.score - e.score || l(e.completion, t.completion))) {
		let t = e.completion;
		!c || c.label != t.label || c.detail != t.detail || c.type != null && t.type != null && c.type != t.type || c.apply != t.apply || c.boost != t.boost ? s.push(e) : me(e.completion) > me(c) && (s[s.length - 1] = e), c = e.completion;
	}
	return s;
}
var ge = class e {
	...
	setSelected(t, n) {
		return t == this.selected || t >= this.options.length ? this : new e(this.options, xe(n, t), this.tooltip, this.timestamp, t, this.disabled);
	}
	static build(t, n, r, i, a, o) {
		...
		let c = n.facet(P).selectOnOpen ? 0 : -1;
	...
	update(t) {
		let { state: n } = t, r = n.facet(P), i = (r.override || n.languageDataAt("autocomplete", A(n)).map(oe)).map((e) => (this.active.find((t) => t.source == e) || new z(e, +!!this.active.some((e) => e.state != 0))).update(t, r));
		i.length == this.active.length && i.every((e, t) => e == this.active[t]) && (i = this.active);
		let a = this.open, o = t.effects.some((e) => e.is(B));
		a && t.docChanged && (a = a.map(t.changes)), t.selection || i.some((e) => e.hasResult() && t.changes.touchesRange(e.from, e.to)) || !ve(i, this.active) || o ? a = ge.build(i, n, this.id, a, r, o) : a && a.disabled && !i.some((e) => e.isPending) && (a = null), !a && i.every((e) => !e.isPending) && i.some((e) => e.hasResult()) && (i = i.map((e) => e.hasResult() ? new z(e.source, 0) : e));
		for (let e of t.effects) e.is(I) && (a &&= a.setSelected(e.value, this.id));
	...
	get attrs() {
		return this.open ? this.open.attrs : this.active.length ? ye : be;
	}
};
function ve(e, t) {
	if (e == t) return !0;
...
var ye = { "aria-autocomplete": "list" }, be = {};
function xe(e, t) {
	let n = {
...
var z = class e {
	...
	update(t, n) {
		...
		for (let n of t.effects) if (n.is(M)) i = new e(i.source, 1, n.value);
		else if (n.is(N)) i = new e(i.source, 0);
		else if (n.is(B)) for (let e of n.value) e.source == i.source && (i = e);
		return i;
	}
	updateFor(e, t) {
	...
	touches(e) {
		return e.changes.touchesRange(A(e.state));
	}
}, Ce = class e extends z {
	constructor(e, t, n, r, i, a) {
	...
	updateFor(t, n) {
		...
		let i = t.changes.mapPos(this.from), a = t.changes.mapPos(this.to, 1), o = A(t.state);
		if (o > a || !r || n & 2 && (A(t.startState) == this.from || o < this.limit)) return new z(this.source, n & 4 ? 1 : 0);
		let s = t.changes.mapPos(this.limit);
		return we(r.validFor, t.state, i, a) ? new e(this.source, this.explicit, s, r, i, a) : r.update && (r = r.update(r, i, a, new E(t.state, o, !1))) ? new e(this.source, this.explicit, s, r, r.from, r.to ?? A(t.state)) : new z(this.source, 1, this.explicit);
	}
	map(t) {
		return t.empty ? this : (this.result.map ? this.result.map(this.result, t) : this.result) ? new e(this.source, this.explicit, t.mapPos(this.limit), this.result, t.mapPos(this.from), t.mapPos(this.to, 1)) : new z(this.source, 0);
	}
	touches(e) {
...
function we(e, t, n, r) {
	...
	return typeof e == "function" ? e(i, n, r, t) : re(e, !0).test(i);
}
var B = /* @__PURE__ */ i.define({ map(e, t) {
	...
	provide: (e) => [y.from(e, (e) => e.tooltip), u.contentAttributes.from(e, (e) => e.attrs)]
});
function H(e, t) {
	let n = t.completion.apply || t.completion.label, r = e.state.field(V).active.find((e) => e.source == t.source);
	return r instanceof Ce ? (typeof n == "string" ? e.dispatch({
		...ie(e.state, n, r.from, r.to),
		annotations: j.of(t.completion)
	}) : n(e, t.completion, r.from, r.to), !0) : !1;
}
var Te = /* @__PURE__ */ fe(V, H);
function U(e, t = "option") {
	return (n) => {
		let r = n.state.field(V, !1);
		if (!r || !r.open || r.open.disabled || Date.now() - r.open.timestamp < n.state.facet(P).interactionDelay) return !1;
		let i = 1, a;
		t == "page" && (a = ee(n, r.open.tooltip)) && (i = Math.max(2, Math.floor(a.dom.offsetHeight / a.dom.querySelector("li").offsetHeight) - 1));
		let { length: o } = r.open.options, s = r.open.selected > -1 ? r.open.selected + i * (e ? 1 : -1) : e ? 0 : o - 1;
		return s < 0 ? s = t == "page" ? 0 : o - 1 : s >= o && (s = t == "page" ? o - 1 : 0), n.dispatch({ effects: I.of(s) }), !0;
	};
}
var Ee = (e) => {
	let t = e.state.field(V, !1);
	return e.state.readOnly || !t || !t.open || t.open.selected < 0 || t.open.disabled || Date.now() - t.open.timestamp < e.state.facet(P).interactionDelay ? !1 : H(e, t.open.options[t.open.selected]);
}, W = (e) => e.state.field(V, !1) ? (e.dispatch({ effects: M.of(!0) }), !0) : !1, De = (e) => {
	let t = e.state.field(V, !1);
	return !t || !t.active.some((e) => e.state != 0) ? !1 : (e.dispatch({ effects: N.of(null) }), !0);
}, Oe = class {
	constructor(e, t) {
		this.active = e, this.context = t, this.time = Date.now(), this.updates = [], this.done = void 0;
	}
}, ke = 50, Ae = 1e3, je = /* @__PURE__ */ e.fromClass(class {
	constructor(e) {
		this.view = e, this.debounceUpdate = -1, this.running = [], this.debounceAccept = -1, this.pendingStart = !1, this.composing = 0;
		for (let t of e.state.field(V).active) t.isPending && this.startQuery(t);
	}
	update(e) {
		let t = e.state.field(V), n = e.state.facet(P);
		if (!e.selectionSet && !e.docChanged && e.startState.field(V) == t) return;
		...
		for (let t = 0; t < this.running.length; t++) {
			...
			if (r || n.context.abortOnDocChange && e.docChanged || n.updates.length + e.transactions.length > ke && Date.now() - n.time > Ae) {
		...
		this.debounceUpdate > -1 && clearTimeout(this.debounceUpdate), e.transactions.some((e) => e.effects.some((e) => e.is(M))) && (this.pendingStart = !0);
	...
	startUpdate() {
		...
		let { state: e } = this.view, t = e.field(V);
		for (let e of t.active) e.isPending && !this.running.some((t) => t.active.source == e.source) && this.startQuery(e);
		this.running.length && t.open && t.open.disabled && (this.debounceAccept = setTimeout(() => this.accept(), this.view.state.facet(P).updateSyncTime));
	}
	startQuery(e) {
		let { state: t } = this.view, n = new E(t, A(t), e.explicit, this.view), r = new Oe(e, n);
		this.running.push(r), Promise.resolve(e.source(n)).then((e) => {
			r.context.aborted || (r.done = e || null, this.scheduleAccept());
		}, (e) => {
			this.view.dispatch({ effects: N.of(null) }), x(this.view.state, e);
		});
	}
	scheduleAccept() {
		this.running.every((e) => e.done !== void 0) ? this.accept() : this.debounceAccept < 0 && (this.debounceAccept = setTimeout(() => this.accept(), this.view.state.facet(P).updateSyncTime));
	}
	accept() {
		this.debounceAccept > -1 && clearTimeout(this.debounceAccept), this.debounceAccept = -1;
		let e = [], t = this.view.state.facet(P), n = this.view.state.field(V);
		for (let r = 0; r < this.running.length; r++) {
			let i = this.running[r];
			if (i.done === void 0) continue;
			if (this.running.splice(r--, 1), i.done) {
				let n = A(i.updates.length ? i.updates[0].startState : this.view.state), r = Math.min(n, i.done.from + +!i.active.explicit), a = new Ce(i.active.source, i.active.explicit, r, i.done, i.done.from, i.done.to ?? n);
		...
		(e.length || n.open && n.open.disabled) && this.view.dispatch({ effects: B.of(e) });
	}
}, { eventHandlers: {
	blur(e) {
		let t = this.view.state.field(V, !1);
		if (t && t.tooltip && this.view.state.facet(P).closeOnBlur) {
			let n = t.open && ee(this.view, t.open.tooltip);
			(!n || !n.dom.contains(e.relatedTarget)) && setTimeout(() => this.view.dispatch({ effects: N.of(null) }), 10);
		}
	},
	compositionstart() {
		this.composing = 1;
	...
	compositionend() {
		this.composing == 3 && setTimeout(() => this.view.dispatch({ effects: M.of(!1) }), 20), this.composing = 0;
	}
} }), Me = typeof navigator == "object" && /* @__PURE__ */ /Win/.test(navigator.platform), Ne = /* @__PURE__ */ S.highest(/* @__PURE__ */ u.domEventHandlers({ keydown(e, t) {
	let n = t.state.field(V, !1);
	if (!n || !n.open || n.open.disabled || n.open.selected < 0 || e.key.length > 1 || e.ctrlKey && !(Me && e.altKey) || e.metaKey) return !1;
	...
	return a && a.indexOf(e.key) > -1 && H(t, r), !1;
} })), G = /* @__PURE__ */ u.baseTheme({
	...
	map(n) {
		let r = n.mapPos(this.from, -1, t.TrackDel), i = n.mapPos(this.to, 1, t.TrackDel);
...
}, Le = /* @__PURE__ */ _.widget({ widget: /* @__PURE__ */ new class extends b {
	toDOM() {
...
}() }), Re = /* @__PURE__ */ _.mark({ class: "cm-snippetField" }), K = class e {
	constructor(e, t) {
		this.ranges = e, this.active = t, this.deco = _.set(e.map((e) => (e.from == e.to ? Le : Re).range(e.from, e.to)), !0);
	}
	map(t) {
		let n = [];
...
}, q = /* @__PURE__ */ i.define({ map(e, t) {
	return e && e.map(t);
} }), ze = /* @__PURE__ */ i.define(), J = /* @__PURE__ */ a.define({
	create() {
		return null;
	},
	update(e, t) {
	...
	provide: (e) => u.decorations.from(e, (e) => e ? e.deco : _.none)
});
function Y(e, t) {
...
function Be(e) {
	let t = Ie.parse(e);
	return (e, n, r, a) => {
		let { text: s, ranges: c } = t.instantiate(e.state, r), { main: l } = e.state.selection, u = {
			changes: {
				from: r,
				to: a == l.from ? l.to : a,
				insert: o.of(s)
			...
			scrollIntoView: !0,
			annotations: n ? [j.of(n), f.userEvent.of("input.complete")] : void 0
		};
		if (c.length && (u.selection = Y(c, 0)), c.some((e) => e.field > 0)) {
			let t = new K(c, 0), n = u.effects = [q.of(t)];
			e.state.field(J, !1) === void 0 && n.push(i.appendConfig.of([
				J,
				We,
				Ke,
				G
...
}], Ue = /* @__PURE__ */ C.define({ combine(e) {
	return e.length ? e[0] : He;
} }), We = /* @__PURE__ */ S.highest(/* @__PURE__ */ w.compute([Ue], (e) => e.facet(Ue)));
function Ge(e, t) {
	return {
		...t,
		apply: Be(e)
...
var Ke = /* @__PURE__ */ u.domEventHandlers({ mousedown(e, t) {
	...
	before: ")]}:;>",
	stringPrefixes: []
}, Z = /* @__PURE__ */ i.define({ map(e, n) {
	return n.mapPos(e, -1, t.TrackAfter) ?? void 0;
} }), Q = /* @__PURE__ */ new class extends l {}();
Q.startSide = 1, Q.endSide = -1;
var qe = /* @__PURE__ */ a.define({
	create() {
		return s.empty;
	},
	update(e, t) {
...
var Qe = typeof navigator == "object" && /* @__PURE__ */ /Android\b/.test(navigator.userAgent), $e = /* @__PURE__ */ u.inputHandler.of((e, t, r, i) => {
	if ((Qe ? e.composing : e.compositionStarted) || e.state.readOnly) return !1;
	let a = e.state.selection.main;
	if (i.length > 2 || i.length == 2 && n(T(i, 0)) == 1 || t != a.from || r != a.to) return !1;
	let o = tt(e.state, i);
	...
	run: ({ state: e, dispatch: t }) => {
		...
		let n = Ze(e, e.selection.main.head).brackets || X.brackets, i = null, a = e.changeByRange((t) => {
			if (t.empty) {
				let i = rt(e.doc, t.head);
				for (let a of n) if (a == i && $(e.doc, t.head) == Xe(T(a, 0))) return {
					changes: {
...
function tt(e, t) {
	let n = Ze(e, e.selection.main.head), r = n.brackets || X.brackets;
	for (let i of r) {
		let a = Xe(T(i, 0));
		if (t == i) return a == i ? ot(e, i, r.indexOf(i + i + i) > -1, n) : it(e, i, a, n.before || X.before);
...
function $(e, t) {
	...
	return r.slice(0, n(T(r, 0)));
}
function rt(e, t) {
	let r = e.sliceString(t - 2, t);
	return n(T(r, 0)) == r.length ? r : r.slice(1);
}
function it(e, t, n, i) {
...
function ot(e, t, n, i) {
	let a = i.stringPrefixes || X.stringPrefixes, o = null, s = e.changeByRange((i) => {
		if (!i.empty) return {
			changes: [{
				insert: t,
				from: i.from
		...
		else if (e.charCategorizer(s)(c) != m.Word && lt(e, s, a) > -1 && !ct(e, s, t, a)) return {
			changes: {
				insert: t + t,
				from: s
			},
...
function st(e, t) {
	let n = d(e).resolveInner(t + 1);
	return n.parent && n.from == t;
}
function ct(e, t, n, r) {
	let i = d(e).resolveInner(t, -1), a = r.reduce((e, t) => Math.max(e, t.length), 0);
	for (let o = 0; o < 5; o++) {
...
var dt = [
	...
	{
		key: "PageDown",
		run: /* @__PURE__ */ U(!0, "page")
	},
	{
		key: "PageUp",
		run: /* @__PURE__ */ U(!1, "page")
	},
	{
		key: "Enter",
		run: Ee
	}
], ft = /* @__PURE__ */ S.highest(/* @__PURE__ */ w.computeN([P], (e) => e.facet(P).defaultKeymap ? [dt] : []));
//#endregion
export { O as a, Ge as c, et as i, ut as n, dt as o, Je as r, ne as s, E as t };

//# sourceMappingURL=dist-DRkDtt-y.js.map