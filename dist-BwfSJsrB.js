import { $ as e, At as t, Ct as n, Dt as r, Ft as i, Ht as a, It as o, Lt as s, Mt as c, N as l, Nt as u, Ot as d, Pt as f, Q as p, R as m, Rt as h, Tt as g, X as _, Z as v, bt as y, dt as b, et as x, kt as S, ot as C, pt as w, vt as ee, zt as te } from "./dist-C56v8Vz7.js";
...
var T = class {
	...
	tokenBefore(e) {
		let t = m(this.state).resolveInner(this.pos, -1);
	...
	matchBefore(e) {
		let t = this.state.doc.lineAt(this.pos), n = Math.max(t.from, this.pos - 250), r = t.text.slice(n - t.from, this.pos - t.from), i = r.search(se(e, !1));
		return i < 0 ? null : {
			from: n + i,
			to: this.pos,
			text: r.slice(i)
...
function ae(e, t) {
	return (n) => {
		for (let t = m(n.state).resolveInner(n.pos, -1); t; t = t.parent) {
	...
	return {
		...e.changeByRange((s) => {
			...
			return {
				...
				range: g.cursor(s.from + a + c.length)
			};
		}),
		scrollIntoView: !0,
...
function le(e) {
	...
	let t = O.get(e);
	return t || O.set(e, t = ie(e)), t;
}
var k = /* @__PURE__ */ u.define(), A = /* @__PURE__ */ u.define(), ue = class {
	constructor(e) {
	...
	match(e) {
		...
		let { chars: t, folded: n, any: r, precise: i, byWord: o } = this;
		if (t.length == 1) {
			let r = s(e, 0), i = h(r), a = i == e.length ? 0 : -100;
			if (r != t[0]) if (r == n[0]) a += -200;
			else return null;
			return this.ret(a, [0, i]);
		}
		let c = e.indexOf(this.pattern);
		if (c == 0) return this.ret(e.length == this.pattern.length ? 0 : -100, [0, this.pattern.length]);
		let l = t.length, u = 0;
		if (c < 0) {
			for (let i = 0, a = Math.min(e.length, 200); i < a && u < l;) {
				let a = s(e, i);
				(a == t[u] || a == n[u]) && (r[u++] = i), i += h(a);
			}
			if (u < l) return null;
		}
		let d = 0, f = 0, p = !1, m = 0, g = -1, _ = -1, v = /[a-z]/.test(e), y = !0;
		for (let r = 0, u = Math.min(e.length, 200), b = 0; r < u && f < l;) {
			let u = s(e, r);
			c < 0 && (d < l && u == t[d] && (i[d++] = r), m < l && (u == t[m] || u == n[m] ? (m == 0 && (g = r), _ = r + 1, m++) : m = 0));
			let x, S = u < 255 ? u >= 48 && u <= 57 || u >= 97 && u <= 122 ? 2 : u >= 65 && u <= 90 ? 1 : 0 : (x = a(u)) == x.toLowerCase() ? x == x.toUpperCase() ? 0 : 2 : 1;
			(!r || S == 1 && v || b == 0 && S != 0) && (t[f] == u || n[f] == u && (p = !0) ? o[f++] = r : o.length && (y = !1)), b = S, r += h(u);
		}
		return f == l && o[0] == 0 && y ? this.result(-100 + (p ? -200 : 0), o, e) : m == l && g == 0 ? this.ret(-200 - e.length + (_ == e.length ? 0 : -100), [0, _]) : c > -1 ? this.ret(-700 - e.length, [c, c + this.pattern.length]) : m == l ? this.ret(-900 - e.length, [g, _]) : f == l ? this.result(-100 + (p ? -200 : 0) + -700 + (y ? 0 : -1100), o, e) : t.length == 2 ? null : this.result((r[0] ? -700 : 0) + -200 + -1100, r, e);
	}
	result(e, t, n) {
		let r = [], i = 0;
		for (let e of t) {
			let t = e + (this.astral ? h(s(n, e)) : 1);
	...
	return te(e, {
		...
		closeOnBlur: (e, t) => e && t,
		icons: (e, t) => e && t,
		tooltipClass: (e, t) => (n) => M(e(n), t(n)),
		optionClass: (e, t) => (n) => M(e(n), t(n)),
...
var me = class {
	constructor(e, t, n) {
		...
		this.optionContent = pe(o), this.optionClass = o.optionClass, this.tooltipClass = o.tooltipClass, this.range = P(i.length, a, o.maxRenderedOptions), this.dom = document.createElement("div"), this.dom.className = "cm-tooltip-autocomplete", this.updateTooltipClass(e.state), this.dom.addEventListener("mousedown", (n) => {
			...
			if (n.target == this.list) {
				...
				t != null && (e.dispatch({ effects: N.of(t) }), n.preventDefault());
			}
		}), this.dom.addEventListener("focusout", (t) => {
			let n = e.state.field(this.stateField, !1);
			n && n.tooltip && e.state.facet(j).closeOnBlur && t.relatedTarget != e.contentDOM && e.dispatch({ effects: A.of(null) });
	...
	update(e) {
		...
		if (this.updateTooltipClass(e.state), t != n) {
			...
			(!n.open || n.open.options != r) && (this.range = P(r.length, i, e.state.facet(j).maxRenderedOptions), this.showOptions(r, t.id)), this.updateSel(), a != n.open?.disabled && this.dom.classList.toggle("cm-tooltip-autocomplete-disabled", !!a);
		}
	}
	...
	updateSel() {
		...
		(t.selected > -1 && t.selected < this.range.from || t.selected >= this.range.to) && (this.range = P(t.options.length, t.selected, this.view.state.facet(j).maxRenderedOptions), this.showOptions(t.options, e.id));
		...
		if (n) {
			...
			}).catch((e) => w(this.view.state, e, "completion info")) : (this.addInfoPane(a, r), n.setAttribute("aria-describedby", this.info.id));
		}
	}
	addInfoPane(e, t) {
		this.destroyInfo();
...
function _e(e, t) {
	...
	for (let r of e) if (r.hasResult()) {
		...
		if (r.result.filter === !1) for (let t of r.result.options) a(new oe(t, r.source, e ? e(t) : [], 1e9 - n.length));
		else {
			let n = t.sliceDoc(r.from, r.to), s, c = o.filterStrict ? new de(n) : new ue(n);
			for (let t of r.result.options) if (s = c.match(t.label)) {
				let n = t.displayLabel ? e ? e(t, s.matched) : [] : s.matched, o = s.score + (t.boost || 0);
				if (a(new oe(t, r.source, n, o)), typeof t.section == "object" && t.section.rank === "dynamic") {
	...
	for (let e of n.sort((e, t) => t.score - e.score || l(e.completion, t.completion))) {
		let t = e.completion;
		!c || c.label != t.label || c.detail != t.detail || c.type != null && t.type != null && c.type != t.type || c.apply != t.apply || c.boost != t.boost ? s.push(e) : F(e.completion) > F(c) && (s[s.length - 1] = e), c = e.completion;
	}
	return s;
}
var ve = class e {
	...
	setSelected(t, n) {
		return t == this.selected || t >= this.options.length ? this : new e(this.options, I(n, t), this.tooltip, this.timestamp, t, this.disabled);
	}
	...
	static start() {
		return new e(Ce, "cm-ac-" + Math.floor(Math.random() * 2e6).toString(36), null);
	}
	update(t) {
		let { state: n } = t, r = n.facet(j), i = (r.override || n.languageDataAt("autocomplete", E(n)).map(le)).map((e) => (this.active.find((t) => t.source == e) || new R(e, this.active.some((e) => e.state != 0) ? 1 : 0)).update(t, r));
		i.length == this.active.length && i.every((e, t) => e == this.active[t]) && (i = this.active);
		let a = this.open, o = t.effects.some((e) => e.is(B));
		a && t.docChanged && (a = a.map(t.changes)), t.selection || i.some((e) => e.hasResult() && t.changes.touchesRange(e.from, e.to)) || !be(i, this.active) || o ? a = ve.build(i, n, this.id, a, r, o) : a && a.disabled && !i.some((e) => e.isPending) && (a = null), !a && i.every((e) => !e.isPending) && i.some((e) => e.hasResult()) && (i = i.map((e) => e.hasResult() ? new R(e.source, 0) : e));
		for (let e of t.effects) e.is(N) && (a &&= a.setSelected(e.value, this.id));
...
var R = class e {
	...
	update(t, n) {
		...
		for (let n of t.effects) if (n.is(k)) i = new e(i.source, 1, n.value);
		else if (n.is(A)) i = new e(i.source, 0);
	...
	updateFor(t, n) {
		...
		let i = t.changes.mapPos(this.from), a = t.changes.mapPos(this.to, 1), o = E(t.state);
		if (o > a || !r || n & 2 && (E(t.startState) == this.from || o < this.limit)) return new R(this.source, n & 4 ? 1 : 0);
		let s = t.changes.mapPos(this.limit);
		return we(r.validFor, t.state, i, a) ? new e(this.source, this.explicit, s, r, i, a) : r.update && (r = r.update(r, i, a, new T(t.state, o, !1))) ? new e(this.source, this.explicit, s, r, r.from, r.to ?? E(t.state)) : new R(this.source, 1, this.explicit);
	}
	map(t) {
		return t.empty ? this : (this.result.map ? this.result.map(this.result, t) : this.result) ? new e(this.source, this.explicit, t.mapPos(this.limit), this.result, t.mapPos(this.from), t.mapPos(this.to, 1)) : new R(this.source, 0);
	}
	touches(e) {
...
function we(e, t, n, r) {
	...
	return typeof e == "function" ? e(i, n, r, t) : se(e, !0).test(i);
}
var B = /* @__PURE__ */ u.define({ map(e, t) {
	...
	provide: (e) => [ee.from(e, (e) => e.tooltip), p.contentAttributes.from(e, (e) => e.attrs)]
});
function H(e, t) {
	let n = t.completion.apply || t.completion.label, r = e.state.field(V).active.find((e) => e.source == t.source);
	return r instanceof z ? (typeof n == "string" ? e.dispatch({
		...ce(e.state, n, r.from, r.to),
		annotations: D.of(t.completion)
	}) : n(e, t.completion, r.from, r.to), !0) : !1;
}
var Te = /* @__PURE__ */ he(V, H);
function U(e, t = "option") {
	return (n) => {
		...
		let i = 1, a;
		t == "page" && (a = C(n, r.open.tooltip)) && (i = Math.max(2, Math.floor(a.dom.offsetHeight / a.dom.querySelector("li").offsetHeight) - 1));
		let { length: o } = r.open.options, s = r.open.selected > -1 ? r.open.selected + i * (e ? 1 : -1) : e ? 0 : o - 1;
		return s < 0 ? s = t == "page" ? 0 : o - 1 : s >= o && (s = t == "page" ? o - 1 : 0), n.dispatch({ effects: N.of(s) }), !0;
	};
}
var Ee = (e) => {
	let t = e.state.field(V, !1);
	return e.state.readOnly || !t || !t.open || t.open.selected < 0 || t.open.disabled || Date.now() - t.open.timestamp < e.state.facet(j).interactionDelay ? !1 : H(e, t.open.options[t.open.selected]);
}, W = (e) => e.state.field(V, !1) ? (e.dispatch({ effects: k.of(!0) }), !0) : !1, De = (e) => {
	let t = e.state.field(V, !1);
	return !t || !t.active.some((e) => e.state != 0) ? !1 : (e.dispatch({ effects: A.of(null) }), !0);
}, Oe = class {
	constructor(e, t) {
	...
	update(e) {
		let t = e.state.field(V), n = e.state.facet(j);
		...
		this.debounceUpdate > -1 && clearTimeout(this.debounceUpdate), e.transactions.some((e) => e.effects.some((e) => e.is(k))) && (this.pendingStart = !0);
	...
	startQuery(e) {
		let { state: t } = this.view, n = new T(t, E(t), e.explicit, this.view), r = new Oe(e, n);
		this.running.push(r), Promise.resolve(e.source(n)).then((e) => {
			r.context.aborted || (r.done = e || null, this.scheduleAccept());
		}, (e) => {
			this.view.dispatch({ effects: A.of(null) }), w(this.view.state, e);
		});
	}
	scheduleAccept() {
	...
	accept() {
		...
		let e = [], t = this.view.state.facet(j), n = this.view.state.field(V);
		for (let r = 0; r < this.running.length; r++) {
			let i = this.running[r];
			if (i.done === void 0) continue;
			if (this.running.splice(r--, 1), i.done) {
				let n = E(i.updates.length ? i.updates[0].startState : this.view.state), r = Math.min(n, i.done.from + (i.active.explicit ? 0 : 1)), a = new z(i.active.source, i.active.explicit, r, i.done, i.done.from, i.done.to ?? n);
	...
	blur(e) {
		...
		if (t && t.tooltip && this.view.state.facet(j).closeOnBlur) {
			let n = t.open && C(this.view, t.open.tooltip);
			(!n || !n.dom.contains(e.relatedTarget)) && setTimeout(() => this.view.dispatch({ effects: A.of(null) }), 10);
		}
	},
	compositionstart() {
		this.composing = 1;
	...
	compositionend() {
		this.composing == 3 && setTimeout(() => this.view.dispatch({ effects: k.of(!1) }), 20), this.composing = 0;
	}
} }), Me = typeof navigator == "object" && /* @__PURE__ */ /Win/.test(navigator.platform), Ne = /* @__PURE__ */ S.highest(/* @__PURE__ */ p.domEventHandlers({ keydown(e, t) {
	...
	map(t) {
		let n = t.mapPos(this.from, -1, d.TrackDel), r = t.mapPos(this.to, 1, d.TrackDel);
...
}, Le = /* @__PURE__ */ _.widget({ widget: /* @__PURE__ */ new class extends x {
	toDOM() {
...
}, q = /* @__PURE__ */ u.define({ map(e, t) {
	return e && e.map(t);
} }), ze = /* @__PURE__ */ u.define(), J = /* @__PURE__ */ f.define({
	create() {
		return null;
	},
	update(e, t) {
	...
	provide: (e) => p.decorations.from(e, (e) => e ? e.deco : _.none)
});
function Y(e, t) {
	return g.create(e.filter((e) => e.field == t).map((e) => g.range(e.from, e.to)));
}
function Be(e) {
	let t = Ie.parse(e);
	return (e, n, r, a) => {
		let { text: s, ranges: c } = t.instantiate(e.state, r), { main: l } = e.state.selection, d = {
			...
			scrollIntoView: !0,
			annotations: n ? [D.of(n), o.userEvent.of("input.complete")] : void 0
		};
		if (c.length && (d.selection = Y(c, 0)), c.some((e) => e.field > 0)) {
			let t = new K(c, 0), n = d.effects = [q.of(t)];
			e.state.field(J, !1) === void 0 && n.push(u.appendConfig.of([
				J,
				We,
				Ke,
				G
...
}], Ue = /* @__PURE__ */ r.define({ combine(e) {
	return e.length ? e[0] : He;
} }), We = /* @__PURE__ */ S.highest(/* @__PURE__ */ b.compute([Ue], (e) => e.facet(Ue)));
function Ge(e, t) {
	return {
		...t,
		apply: Be(e)
...
var Ke = /* @__PURE__ */ p.domEventHandlers({ mousedown(e, t) {
	...
	before: ")]}:;>",
	stringPrefixes: []
}, Z = /* @__PURE__ */ u.define({ map(e, t) {
	return t.mapPos(e, -1, d.TrackAfter) ?? void 0;
} }), Q = /* @__PURE__ */ new class extends c {}();
Q.startSide = 1, Q.endSide = -1;
var qe = /* @__PURE__ */ f.define({
	create() {
		return t.empty;
	},
	update(e, t) {
...
function Xe(e) {
	...
	return a(e < 128 ? e : e + 1);
}
function Ze(e, t) {
	return e.languageDataAt("closeBrackets", t)[0] || X;
}
var Qe = typeof navigator == "object" && /* @__PURE__ */ /Android\b/.test(navigator.userAgent), $e = /* @__PURE__ */ p.inputHandler.of((e, t, n, r) => {
	if ((Qe ? e.composing : e.compositionStarted) || e.state.readOnly) return !1;
	let i = e.state.selection.main;
	if (r.length > 2 || r.length == 2 && h(s(r, 0)) == 1 || t != i.from || n != i.to) return !1;
	let a = tt(e.state, r);
	...
	run: ({ state: e, dispatch: t }) => {
		...
		let n = Ze(e, e.selection.main.head).brackets || X.brackets, r = null, i = e.changeByRange((t) => {
			if (t.empty) {
				let r = rt(e.doc, t.head);
				for (let i of n) if (i == r && $(e.doc, t.head) == Xe(s(i, 0))) return {
					changes: {
						from: t.head - i.length,
						to: t.head + i.length
					},
					range: g.cursor(t.head - i.length)
				};
			}
			return { range: r = t };
		});
...
function $(e, t) {
	...
	return n.slice(0, h(s(n, 0)));
}
function rt(e, t) {
	let n = e.sliceString(t - 2, t);
	return h(s(n, 0)) == n.length ? n : n.slice(1);
}
function it(e, t, n, r) {
	let i = null, a = e.changeByRange((a) => {
		if (!a.empty) return {
			...
			effects: Z.of(a.to + t.length),
			range: g.range(a.anchor + t.length, a.head + t.length)
		};
		let o = $(e.doc, a.head);
		return !o || /\s/.test(o) || r.indexOf(o) > -1 ? {
			...
			effects: Z.of(a.head + t.length),
			range: g.cursor(a.head + t.length)
		} : { range: i = a };
	});
...
function at(e, t, n) {
	let r = null, i = e.changeByRange((t) => t.empty && $(e.doc, t.head) == n ? {
		changes: {
			from: t.head,
			to: t.head + n.length,
			insert: n
		},
		range: g.cursor(t.head + n.length)
	} : r = { range: t });
...
function ot(e, t, r, i) {
	let a = i.stringPrefixes || X.stringPrefixes, o = null, s = e.changeByRange((i) => {
		if (!i.empty) return {
			...
			range: g.range(i.anchor + t.length, i.head + t.length)
		};
		...
		if (c == t) {
			if (st(e, s)) return {
				...
				range: g.cursor(s + t.length)
			};
			if (nt(e, s)) {
				let n = r && e.sliceDoc(s, s + t.length * 3) == t + t + t ? t + t + t : t;
				return {
					changes: {
						from: s,
						to: s + n.length,
					...
					range: g.cursor(s + n.length)
				};
			}
		} else if (r && e.sliceDoc(s - 2 * t.length, s) == t + t && (l = lt(e, s - 2 * t.length, a)) > -1 && st(e, l)) return {
			changes: {
				insert: t + t + t + t,
				from: s
			},
		...
		else if (e.charCategorizer(s)(c) != n.Word && lt(e, s, a) > -1 && !ct(e, s, t, a)) return {
			changes: {
				insert: t + t,
				from: s
			},
...
function st(e, t) {
	let n = m(e).resolveInner(t + 1);
	return n.parent && n.from == t;
}
function ct(e, t, n, r) {
	let i = m(e).resolveInner(t, -1), a = r.reduce((e, t) => Math.max(e, t.length), 0);
	for (let o = 0; o < 5; o++) {
...
function lt(e, t, r) {
	...
	if (i(e.sliceDoc(t - 1, t)) != n.Word) return t;
	for (let a of r) {
		let r = t - a.length;
		if (e.sliceDoc(r, t) == a && i(e.sliceDoc(r - 1, r)) != n.Word) return r;
	}
	return -1;
}
function ut(e = {}) {
...
//# sourceMappingURL=dist-BwfSJsrB.js.map