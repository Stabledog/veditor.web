import { A as e, B as t, E as n, Q as r, R as i, Tt as a, V as o, h as s, n as c, r as l, s as u, t as d, u as f } from "./dist-C56v8Vz7.js";
...
var A = new d({
	start: null,
	shift(e, t, n, r) {
		return t == p ? new k(O(r, 1) || "", e) : e;
	},
	reduce(e, t) {
		return t == x && e ? e.parent : e;
	},
	reuse(e, t, n, r) {
		let i = t.type.id;
		return i == p || i == S ? new k(O(r, 1) || "", e) : e;
	},
	strict: !1
}), j = new c((e, t) => {
	if (e.next == 60) {
		if (e.advance(), e.next == 47) {
			e.advance();
			let n = O(e, 0);
			if (!n) return e.acceptToken(_);
			if (t.context && n == t.context.name) return e.acceptToken(m);
			for (let r = t.context; r; r = r.parent) if (r.name == n) return e.acceptToken(h, -2);
			e.acceptToken(g);
		} else if (e.next != 33 && e.next != 63) return e.acceptToken(p);
	}
}, { contextual: !0 });
function M(e, t) {
	return new c((n) => {
...
var N = M(v, "-->"), P = M(y, "?>"), F = M(b, "]]>"), I = t({
	Text: o.content,
	"StartTag StartCloseTag EndTag SelfCloseEndTag": o.angleBracket,
	TagName: o.tagName,
...
function H(e, t) {
	let n = i(e).resolveInner(t, -1), r = null;
...
function q(e, t) {
	...
	return (e) => {
		let { doc: t } = e.state, n = H(e.state, e.pos);
		...
		if (c == "openTag") {
			...
			return n ? {
				from: l,
				to: e.pos + (t.sliceString(e.pos, e.pos + 1) == ">" ? 1 : 0),
			...
			from: l,
			options: s[R(t, u)]?.attrs || r,
			validFor: W
		};
		else if (c == "attrValue") {
			let n = B(t, u, l);
			if (!n) return null;
			let r = (s[R(t, u)]?.attrValues || i)[n];
			...
			let n = z(t, u), r = s[n], i = [], c = u && u.lastChild;
			n && (!c || c.name != "CloseTag" || R(t, c) != n) && i.push(r ? r.closeCompletion : {
				label: "</" + n + ">",
				type: "type",
				boost: 2
			});
...
var J = /* @__PURE__ */ u.define({
	name: "xml",
	parser: /* @__PURE__ */ L.configure({ props: [
		/* @__PURE__ */ e.add({
			Element(e) {
		...
		/* @__PURE__ */ s.add({ "OpenTag CloseTag": (e) => e.getChild("TagName") })
	] }),
	languageData: {
		commentTokens: { block: {
			open: "<!--",
...
function Y(e = {}) {
	let t = [J.data.of({ autocomplete: q(e.elements || [], e.attributes || []) })];
	return e.autoCloseTags !== !1 && t.push(Z), new f(J, t);
}
...
var Z = /* @__PURE__ */ r.inputHandler.of((e, t, n, r, o) => {
	if (e.composing || e.state.readOnly || t != n || r != ">" && r != "/" || !J.isActiveAt(e.state, t, -1)) return !1;
	let s = o(), { state: c } = s, l = c.changeByRange((e) => {
		let { head: t } = e, n = c.doc.sliceString(t - 1, t) == r, o = i(c).resolveInner(t, -1), s;
		if (n && r == ">" && o.name == "EndTag") {
			let n = o.parent;
			if (n.parent?.lastChild?.name != "CloseTag" && (s = X(c.doc, n.parent, t))) return {
				range: e,
				changes: {
					from: t,
					to: t + (c.doc.sliceString(t, t + 1) === ">" ? 1 : 0),
					insert: `</${s}>`
				}
			};
		} else if (n && r == "/" && o.name == "StartCloseTag") {
			let e = o.parent;
			if (o.from == t - 2 && e.lastChild?.name != "CloseTag" && (s = X(c.doc, e, t))) {
				let e = t + (c.doc.sliceString(t, t + 1) === ">" ? 1 : 0), n = `${s}>`;
				return {
					range: a.cursor(t + n.length, -1),
					changes: {
						from: t,
						to: e,
						insert: n
...
//# sourceMappingURL=dist-CvVaTbhs.js.map