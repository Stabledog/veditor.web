import { A as e, B as t, E as n, V as r, g as i, n as a, r as o, s, t as c, u as l, w as u } from "./dist-CVKUUr4R.js";
import { r as d } from "./dist-DVlZlzMH.js";
//#region node_modules/@lezer/sass/dist/index.js
var f = 168, p = 169, m = 170, h = 1, ee = 2, g = 3, _ = 171, v = 172, y = 4, b = 173, x = 5, S = 174, C = 175, w = 176, T = 177, E = 6, D = 7, te = 8, ne = 9, O = 0, k = [
	9,
	10,
	11,
	12,
	...
	8287,
	12288
], A = 58, j = 40, M = 95, N = 91, P = 45, F = 46, I = 35, L = 37, R = 123, z = 125, B = 47, V = 42, H = 10, U = 61, W = 43, G = 38;
function K(e) {
...
function J(e) {
	let t;
	return e.next == B && ((t = e.peek(1)) == B || t == V);
}
var re = new a((e, t) => {
	if (t.dialectEnabled(O)) {
		let n;
		if (e.next < 0 && t.canShift(w)) e.acceptToken(w);
		else if (((n = e.peek(-1)) == H || n < 0) && t.canShift(C)) {
			let t = 0;
			for (; e.next != H && k.includes(e.next);) e.advance(), t++;
			e.next == H || J(e) ? e.acceptToken(C, -t) : t && e.acceptToken(T);
		} else if (e.next == H) e.acceptToken(S, 1);
		else if (k.includes(e.next)) {
			for (e.advance(); e.next != H && k.includes(e.next);) e.advance();
			e.acceptToken(T);
		}
	} else {
		let t = 0;
		for (; k.includes(e.next);) e.advance(), t++;
		t && e.acceptToken(T);
	}
}, { contextual: !0 }), Y = new a((e, t) => {
	if (J(e)) {
		if (e.advance(), t.dialectEnabled(O)) {
			let t = -1;
			for (let n = 1;; n++) {
				let r = e.peek(-n - 1);
				if (r == H || r < 0) {
				...
				if (!k.includes(r)) break;
			}
			if (t > -1) {
				let n = e.next == V, r = 0;
				for (e.advance(); e.next >= 0;) if (e.next == H) {
					e.advance();
					let n = 0;
					for (; e.next != H && k.includes(e.next);) n++, e.advance();
					if (n < t) {
						r = -n - 1;
						break;
					}
				} else if (n && e.next == V && e.peek(1) == B) {
					r = 2;
					break;
				} else e.advance();
		...
		if (e.next == B) {
			for (; e.next != H && e.next >= 0;) e.advance();
			e.acceptToken(E);
		} else {
			for (e.advance(); e.next >= 0;) {
				let { next: t } = e;
				if (e.advance(), t == V && e.next == B) {
					e.advance();
					break;
				}
			}
	...
	(e.next == W || e.next == U) && t.dialectEnabled(O) && e.acceptToken(e.next == U ? te : ne, 1);
}), ie = new a((e, t) => {
	if (!t.dialectEnabled(O)) return;
	...
	if (e.peek(-1) == H) {
		let t = 0;
		for (; e.next != H && k.includes(e.next);) e.advance(), t++;
		t != n && e.next != H && !J(e) && (t < n ? e.acceptToken(p, -t) : e.acceptToken(f));
	}
}), ae = new a((e, t) => {
	for (let n = !1, r = 0, i = 0;; i++) {
		let { next: a } = e;
		if (K(a) || a == P || a == M || n && q(a)) !n && (a != P || i > 0) && (n = !0), r === i && a == P && r++, e.advance();
		else if (a == I && e.peek(1) == R) {
			e.acceptToken(x, 2);
			break;
		} else {
			n && e.acceptToken(r == 2 && t.canShift(y) ? y : t.canShift(b) ? b : a == j ? _ : v);
			break;
		}
	}
}), oe = new a((e) => {
	if (e.next == z) {
		for (e.advance(); K(e.next) || e.next == P || e.next == M || q(e.next);) e.advance();
		e.next == I && e.peek(1) == R ? e.acceptToken(ee, 2) : e.acceptToken(h);
	}
}), se = new a((e) => {
	if (k.includes(e.peek(-1))) {
		let { next: t } = e;
		(K(t) || t == M || t == I || t == F || t == N || t == A && K(e.peek(1)) || t == P || t == G || t == V) && e.acceptToken(m);
	}
}), ce = new a((e) => {
	if (!k.includes(e.peek(-1))) {
		let { next: t } = e;
		if (t == L && (e.advance(), e.acceptToken(g)), K(t)) {
			do
				e.advance();
			while (K(e.next) || q(e.next));
			e.acceptToken(g);
		}
	}
});
...
var le = new Z(null, 0), ue = new c({
	start: le,
	shift(e, t, n, r) {
		return t == f ? new Z(e, n.pos - r.pos) : t == p ? e.parent : e;
	},
	hash(e) {
		return e.hash;
	}
}), de = t({
	"AtKeyword import charset namespace keyframes media supports include mixin use forward extend at-root": r.definitionKeyword,
	...
	name: "sass",
	parser: /*@__PURE__*/ he.configure({ props: [/*@__PURE__*/ n.add({
		Block: u,
		Comment(e, t) {
			return {
				from: e.from + 2,
	...
	}), /*@__PURE__*/ e.add({ Declaration: /*@__PURE__*/ i() })] }),
	languageData: {
		commentTokens: {
			block: {
				open: "/*",
...
}), ge = /*@__PURE__*/ $.configure({
	dialect: "indented",
	props: [/*@__PURE__*/ e.add({ "Block RuleSet": (e) => e.baseIndent + e.unit }), /*@__PURE__*/ n.add({ Block: (e) => ({
		from: e.from,
		to: e.to
	}) })]
}), _e = /*@__PURE__*/ d((e) => e.name == "VariableName" || e.name == "SassVariableName");
function ve(e) {
	return new l(e?.indented ? ge : $, $.data.of({ autocomplete: _e }));
}
//#endregion
export { ve as sass };

//# sourceMappingURL=dist-_LS1JZel.js.map