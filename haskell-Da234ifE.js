...
function u(u, p) {
	if (u.eatWhile(l)) return null;
	var m = u.next();
	if (c.test(m)) {
		if (m == "{" && u.eat("-")) {
			var h = "comment";
			return u.eat("#") && (h = "meta"), e(u, p, d(h, 1));
		}
		return null;
	}
	if (m == "'") return u.eat("\\"), u.next(), u.eat("'") ? "string" : "error";
	if (m == "\"") return e(u, p, f);
	if (n.test(m)) return u.eatWhile(o), u.eat(".") ? "qualifier" : "type";
	if (t.test(m)) return u.eatWhile(o), "variable";
	if (r.test(m)) {
		if (m == "0") {
			if (u.eat(/[xX]/)) return u.eatWhile(i), "integer";
			if (u.eat(/[oO]/)) return u.eatWhile(a), "number";
		}
		u.eatWhile(r);
		var h = "number";
		return u.match(/^\.\d+/) && (h = "number"), u.eat(/[eE]/) && (h = "number", u.eat(/[-+]/), u.eatWhile(r)), h;
	}
	return m == "." && u.eat(".") ? "keyword" : s.test(m) ? m == "-" && u.eat(/-/) && (u.eatWhile(/-/), !u.eat(s)) ? (u.skipToEnd(), "comment") : (u.eatWhile(s), "variable") : "error";
}
function d(e, t) {
	return t == 0 ? u : function(n, r) {
		for (var i = t; !n.eol();) {
			var a = n.next();
			if (a == "{" && n.eat("-")) ++i;
			else if (a == "-" && n.eat("}") && (--i, i == 0)) return r(u), e;
		}
		return r(d(e, i)), e;
	};
}
function f(e, t) {
	for (; !e.eol();) {
		...
		if (n == "\\") {
			if (e.eol() || e.eat(l)) return t(p), "string";
			e.eat("&") || e.next();
		}
	}
	return t(u), "error";
...
function p(t, n) {
	return t.eat("\\") ? e(t, n, f) : (t.next(), n(u), "error");
}
var m = (function() {
	var e = {};
	function t(t) {
...
//# sourceMappingURL=haskell-Da234ifE.js.map