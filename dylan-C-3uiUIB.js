...
function f(e, n) {
	var r = e.peek();
	if (r == "'" || r == "\"") return e.next(), d(e, n, m(r, "string"));
	if (r == "/") {
		if (e.next(), e.eat("*")) return d(e, n, p);
		if (e.eat("/")) return e.skipToEnd(), "comment";
		e.backUp(1);
	} else if (/[+\-\d\.]/.test(r)) {
		if (e.match(/^[+-]?[0-9]*\.[0-9]*([esdx][+-]?[0-9]+)?/i) || e.match(/^[+-]?[0-9]+([esdx][+-]?[0-9]+)/i) || e.match(/^[+-]?\d+/)) return "number";
	} else if (r == "#") return e.next(), r = e.peek(), r == "\"" ? (e.next(), d(e, n, m("\"", "string"))) : r == "b" ? (e.next(), e.eatWhile(/[01]/), "number") : r == "x" ? (e.next(), e.eatWhile(/[\da-f]/i), "number") : r == "o" ? (e.next(), e.eatWhile(/[0-7]/), "number") : r == "#" ? (e.next(), "punctuation") : r == "[" || r == "(" ? (e.next(), "bracket") : e.match(/f|t|all-keys|include|key|next|rest/i) ? "atom" : (e.eatWhile(/[-a-zA-Z]/), "error");
	else if (r == "~") return e.next(), r = e.peek(), r == "=" ? (e.next(), r = e.peek(), r == "=" && e.next(), "operator") : "operator";
	else if (r == ":") {
		if (e.next(), r = e.peek(), r == "=") return e.next(), "operator";
		if (r == ":") return e.next(), "punctuation";
	} else if ("[](){}".indexOf(r) != -1) return e.next(), "bracket";
	else if (".,".indexOf(r) != -1) return e.next(), "punctuation";
	else if (e.match("end")) return "keyword";
	for (var s in a) if (a.hasOwnProperty(s)) {
		var c = a[s];
		if (c instanceof Array && t(c, function(t) {
			return e.match(t);
		}) || e.match(c)) return o[s];
	}
	return /[+\-*\/^=<>&|]/.test(r) ? (e.next(), "operator") : e.match("define") ? "def" : (e.eatWhile(/[\w\-]/), l.hasOwnProperty(e.current()) ? u[e.current()] : e.current().match(i) ? "variable" : (e.next(), "variableName.standard"));
}
function p(e, t) {
...
//# sourceMappingURL=dylan-C-3uiUIB.js.map