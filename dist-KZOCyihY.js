import { A as e, B as t, Dt as n, E as r, Q as i, R as a, V as o, Y as s, i as c, n as l, r as u, s as d, u as f, y as p } from "./dist-1T0MK0Nn.js";
import { n as m } from "./dist-DjI0KX1F.js";
//#region node_modules/@codemirror/lang-jinja/dist/index.js
var h = 1, g = 2, _ = 3, v = 155, y = 4, b = 156;
function x(e) {
	return e >= 65 && e <= 90 || e >= 97 && e <= 122;
}
var S = /* @__PURE__ */ new l((e) => {
	let t = e.pos;
	for (;;) {
		let { next: n } = e;
		if (n < 0) break;
...
var w = /* @__PURE__ */ C("endraw", b, y), T = {
	...
	tokenizers: [
		...
		4,
		5,
		/* @__PURE__ */ new c("b~RPstU~XP#q#r[~aO$Q~~", 17, 173)
	],
...
var k = /* @__PURE__ */ O("abs attr batch capitalize center default dictsort escape filesizeformat first float forceescape format groupby indent int items join last length list lower map max min pprint random reject rejectattr replace reverse round safe select selectattr slice sort string striptags sum title tojson trim truncate unique upper urlencode urlize wordcount wordwrap xmlattr", "function"), A = /* @__PURE__ */ O("boolean callable defined divisibleby eq escaped even filter float ge gt in integer iterable le lower lt mapping ne none number odd sameas sequence string test undefined upper range lipsum dict joiner namespace", "function"), j = /* @__PURE__ */ O("loop super self true false varargs kwargs caller name arguments catch_kwargs catch_varargs caller", "keyword"), M = /* @__PURE__ */ A.concat(j), N = /* @__PURE__ */ O("raw endraw filter endfilter trans pluralize endtrans with endwith autoescape endautoescape if elif else endif for endfor call endcall block endblock set endset macro endmacro import include break continue debug do extends", "keyword");
function P(e) {
	let { state: t, pos: n } = e, r = a(t).resolveInner(n, -1).enterUnfinishedNodesBefore(n), i = r.childBefore(n)?.name || r.name;
	if (r.name == "FilterName") return {
		type: "filter",
		node: r
	};
	if (e.explicit && (i == "FilterOp" || i == "filter")) return { type: "filter" };
	if (r.name == "TagName") return {
		type: "tag",
		node: r
	};
	if (e.explicit && i == "{%") return { type: "tag" };
	if (r.name == "PropertyName" && r.parent.name == "MemberExpression") return {
		type: "prop",
		node: r,
		target: r.parent
	};
	if (r.name == "." && r.parent.name == "MemberExpression") return {
		type: "prop",
		target: r.parent
	};
	if (r.name == "MemberExpression" && i == ".") return {
		type: "prop",
		target: r
	};
	if (r.name == "VariableName") return {
		type: "expr",
		from: r.from
	};
	if (r.name == "Comment" || r.name == "StringLiteral" || r.name == "NumberLiteral") return null;
	let o = e.matchBefore(/[\w\u00c0-\uffff]+$/);
	return o ? {
		type: "expr",
		from: o.from
	} : e.explicit ? { type: "expr" } : null;
}
function F(e, t, n, r) {
	let i = [];
	for (;;) {
...
function I(e = {}) {
	let t = e.tags ? e.tags.concat(N) : N, n = e.variables ? e.variables.concat(M) : M, { properties: r } = e;
	return (e) => {
		let i = P(e);
		if (!i) return null;
		let a = i.from ?? (i.node ? i.node.from : e.pos), o;
		return o = i.type == "filter" ? k : i.type == "tag" ? t : i.type == "expr" ? n : r ? F(e.state, i.target, e, r) : [], o.length ? {
			options: o,
			from: a,
			validFor: /^[\w\u00c0-\uffff]*$/
		} : null;
...
var L = /* @__PURE__ */ i.inputHandler.of((e, t, r, i) => i != "%" || t != r || e.state.doc.sliceString(t - 1, r + 1) != "{}" ? !1 : (e.dispatch(e.state.changeByRange((e) => ({
	changes: {
		from: e.from,
		to: e.to,
		insert: "%%"
	},
	range: n.cursor(e.from + 1)
})), {
	scrollIntoView: !0,
...
var z = /* @__PURE__ */ d.define({
	name: "jinja",
	parser: /* @__PURE__ */ D.configure({ props: [
		/* @__PURE__ */ t({
			"TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape": o.keyword,
			"required scoped recursive with without context ignore missing": o.modifier,
			self: o.self,
			"loop super": /* @__PURE__ */ o.standard(o.variableName),
			"if elif else endif for endfor call endcall": o.controlKeyword,
			"block endblock set endset macro endmacro import from include": o.definitionKeyword,
			"Comment/...": o.blockComment,
			VariableName: o.variableName,
			Definition: /* @__PURE__ */ o.definition(o.variableName),
			PropertyName: o.propertyName,
			FilterName: /* @__PURE__ */ o.special(o.variableName),
			ArithOp: o.arithmeticOperator,
			AssignOp: o.definitionOperator,
			"not and or": o.logicOperator,
			CompareOp: o.compareOperator,
			"in is": o.operatorKeyword,
			"FilterOp ConcatOp": o.operator,
		...
		/* @__PURE__ */ e.add({
			Tag: /* @__PURE__ */ p({ closing: "%}" }),
			"IfStatement ForStatement": /* @__PURE__ */ R(/^\s*(\{%-?\s*)?(endif|endfor|else|elif)\b/),
			Statement: /* @__PURE__ */ R(/^\s*(\{%-?\s*)?end\w/)
		}),
		/* @__PURE__ */ r.add({ "Statement Comment"(e) {
			let t = e.firstChild, n = e.lastChild;
			return !t || t.name != "Tag" && t.name != "{#" ? null : {
				from: t.to,
				to: n.name == "EndTag" || n.name == "#}" ? n.from : e.to
			};
		} })
	] }),
...
function V(e) {
	return z.configure({ wrap: s((t) => t.type.isTop ? {
		parser: e.parser,
		overlay: (e) => e.name == "Text" || e.name == "RawText"
	} : null) }, "jinja");
}
var H = /* @__PURE__ */ V(B.language);
function U(e = {}) {
	let t = e.base || B, n = t.language == B.language ? H : V(t.language);
	return new f(n, [
		t.support,
		n.data.of({ autocomplete: I(e) }),
...
//# sourceMappingURL=dist-KZOCyihY.js.map