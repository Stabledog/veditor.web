...
function Ge(e) {
	...
	function O(e, t, n) {
		...
		return (i !== "global" && t && t.state.vim.options[e] || i !== "local" && r || {}).value;
	}
	D("filetype", void 0, "string", ["ft"], function(e, t) {
		if (t !== void 0) {
			if (e === void 0) {
	...
	}), D("textwidth", 80, "number", ["tw"], function(e, t) {
		if (t !== void 0) {
	...
	class be {
		...
		pushText(e, t, n, r, i) {
			if (e !== "_") {
				...
				S(e) ? a.pushText(n, r) : a.setText(n, r, i), e === "+" && navigator.clipboard.writeText(n), this.unnamedRegister.setText(a.toString(), r);
			}
		}
		getRegister(e) {
	...
	var Se = {
		...
		processCommand: function(e, t, n) {
			switch (t.inputState.repeatOverride = n.repeatOverride, n.type) {
				...
				case "search":
					...
					break;
				case "ex":
				case "keyToEx": this.processEx(e, t, n);
			}
		},
		...
		processEx: function(t, n, r) {
			...
			if (r.type == "keyToEx") W.processCommand(t, r.exArgs.input);
				...
				if (n.visualMode) s.value = "'<,'>", s.selectValueOnOpen = !1;
				else {
		...
		evalInput: function(e, r) {
			...
			if (s) {
				if (c.lastSel) {
					g = m;
					var x = c.lastSel, S = Math.abs(x.head.line - x.anchor.line), C = Math.abs(x.head.ch - x.anchor.ch);
		...
		moveByDisplayLines: function(e, t, n, r) {
			...
			for (var a = Math.round(n.repeat), o = 0; o < a; o++) {
				var s = e.findPosV(i, n.forward ? 1 : -1, "line", r.lastHSPos);
				if (s.hitSide) break;
				i = s;
			}
			return i != t && (r.lastHPos = i.ch), i;
		},
		...
		textObjectManipulation: function(e, t, n, r) {
			...
			if (i[o]) {
				...
				if (c = ft(e, t, n.repeat, 0, s), n.linewise = !0, r.visualMode) r.visualLine ||= !0;
				else {
				...
				t.ch > 0 && w(m[t.ch]) && --t.ch;
				var h = pt(e, t, n.repeat, 1, s), g = pt(e, t, n.repeat, -1, s);
				C(e.getLine(g.line)[g.ch]) && C(e.getLine(h.line)[h.ch - 1]) && (g = {
					line: g.line,
					ch: g.ch + 1
				}), c = {
					start: g,
	...
	var Te = {
		...
		indent: function(e, t, n) {
			...
			if (e.indentMore) for (var d = 0; d < i; d++) t.indentRight ? e.indentMore() : e.indentLess();
			else {
		...
		toggleComment: function(e, t, n, r, i) {
			return e.execCommand("toggleLineComment"), i;
		},
		changeCase: function(e, t, n, r, i) {
			for (var a = e.getSelections(), o = [], s = t.toLower, c = 0; c < a.length; c++) {
				...
				if (s === !0) u = l.toLowerCase();
					...
					var f = l.charAt(d);
					u += S(f) ? f.toLowerCase() : f.toUpperCase();
				}
				o.push(u);
			}
	...
	var De = {
		...
		continuePaste: function(e, n, r, i, a) {
			...
			if (i) {
				...
				var p = n.linewise == null ? a.linewise : n.linewise, m = a.blockwise, h = m ? i.split("\n") : void 0;
				if (h) {
					p && h.pop();
	...
	function je(e, t) {
		...
		if (n || r) {
			...
			return a == o && e.length > i ? "full" : o.indexOf(a) == 0 && "partial";
		}
	...
	function Qe(e, { inclusive: n, innerWord: r, bigWord: i, noSymbol: a, multiline: o }, s) {
		...
		if (n) {
			...
			if (b == v || x) {
				for (var S = y; /\s/.test(l.charAt(y - 1)) && y > 0;) y--;
				!y && !x && (y = S);
			}
		}
		return {
	...
	function dt(e, t, n, r, i) {
		var a;
		return r ? (a = t.indexOf(n, e + 1), a != -1 && !i && --a) : (a = t.lastIndexOf(n, e - 1), a != -1 && !i && (a += 1)), a;
	}
	...
	function pt(e, n, r, i, a) {
		...
		function s(e, t, n, r) {
			...
			var s = i.pos;
			for (o(i); i.line !== null;) {
				if (s = i.pos, w(i.line[i.pos])) {
					if (a) {
						for (o(i); i.line !== null && C(i.line[i.pos]);) s = i.pos, o(i);
						return {
							ln: i.ln,
							pos: s + 1
						};
		...
		function c(e, t, n, r) {
			...
			var c = s.pos;
			for (o(s); s.line !== null;) {
				if (!C(s.line[s.pos]) && !w(s.line[s.pos])) c = s.pos;
				else if (w(s.line[s.pos])) return a && C(s.line[s.pos + 1]) ? {
					ln: s.ln,
					pos: s.pos + 1
				} : {
					ln: s.ln,
			...
			return s.line = i, a && C(s.line[s.pos]) ? {
				ln: s.ln,
				pos: s.pos
			} : {
				ln: s.ln,
	...
	function mt(e, n, r, i) {
		...
		function o(e, t, n, r) {
			...
			for (a(e, s); s.line !== null;) {
				if (c.ln = s.ln, c.pos = s.pos, s.line === "" && !l || o && s.line !== "" && !C(s.line[s.pos])) return {
					ln: s.ln,
					pos: s.pos
				};
				w(s.line[s.pos]) && !o && (s.pos === s.line.length - 1 || C(s.line[s.pos + 1])) && (o = !0), a(e, s);
			}
			var i = e.getLine(c.ln);
			c.pos = 0;
			for (var u = i.length - 1; u >= 0; --u) if (!C(i[u])) {
				c.pos = u;
				break;
			}
			return c;
		...
		function s(e, t, n, r) {
			...
			for (a(e, o); o.line !== null;) {
				...
				if (w(o.line[o.pos]) && c !== null && (o.ln !== s || o.pos + 1 !== c)) return {
					ln: s,
					pos: c
				};
				o.line !== "" && !C(o.line[o.pos]) && (l = !1, s = o.ln, c = o.pos), a(e, o);
			}
			var i = e.getLine(s);
			c = 0;
			for (var u = 0; u < i.length; ++u) if (!C(i[u])) {
				c = u;
				break;
			}
			return {
	...
	function kt(e) {
		...
		for (var t = 1; t < arguments.length; t++) {
			var n = arguments[t];
			if (n) {
				if (typeof n != "object" && (n = document.createTextNode(n)), n.nodeType) e.appendChild(n);
	...
	class Wt {
		...
		parseInput_(e, t, n) {
			if (t.eatWhile(":"), t.eat("%")) n.line = e.firstLine(), n.lineEnd = e.lastLine();
			else if (t.eat("*")) {
		...
		parseLineSpec_(e, t) {
			var n = t.match(/^([\d]+)/);
			if (n) return this.parseLineSpecOffset_(e, t, parseInt(n[1], 10) - 1);
			switch (t.next()) {
				...
				case "+":
				case "/":
				case "?": return t.backUp(1), this.parseLineSpecOffset_(e, t, e.getCursor().line);
				default:
					t.backUp(1);
					return;
			}
		}
		parseLineSpecOffset_(e, n, r) {
			for (;;) {
				var i = n.match(/^([\/\?]|\\[\?\/])|([+-]?)(\d*)/);
				if (!i || !i[0]) break;
				if (i[1]) {
					var a = "", o = !i[1].endsWith("?");
					if (i[1].length == 1) {
						var s = n.match(o ? /^([^\/\\]|\\\/)*/ : /^([^\/\?]|\\\?)*/);
						n.match(o ? /^\/?/ : /^\??/), a = s && s[0];
					}
					...
					var u = l.from();
					if (!u) throw Error("Pattern not found" + c);
					r = u.line;
				} else {
					var d = parseInt(i[2] + (i[3] || "1"), 10);
					r += d;
				}
			}
			return r;
		...
		sort: function(n, r) {
			...
			function u() {
				if (r.argString) {
					...
					var n = t.match(/([dinuoxr]+)?\s*(\/.+\/)?\s*/);
					if (!n || !t.eol()) return "Invalid arguments";
					if (n[1]) {
						l = n[1].indexOf("r") != -1, a = n[1].indexOf("i") != -1, o = n[1].indexOf("u") != -1;
						var u = n[1].indexOf("d") != -1 || n[1].indexOf("n") != -1, d = n[1].indexOf("x") != -1, f = n[1].indexOf("o") != -1;
						if (Number(u) + Number(d) + Number(f) > 1) return "Invalid arguments";
						s = u && "decimal" || d && "hex" || f && "octal";
					}
			...
			function C(e, t) {
				...
				var r = _ && _.exec(e), o = _ && _.exec(t);
				return !r || !o ? e < t ? -1 : 1 : parseInt((r[1] + r[2]).toLowerCase(), v) - parseInt((o[1] + o[2]).toLowerCase(), v);
			}
			function w(e, t) {
				if (i) {
					var n = e;
			...
			if (y.sort(c ? w : C), c) for (var x = 0; x < y.length; x++) y[x] = y[x].input;
			else s || b.sort(C);
			if (g = i ? y.concat(b) : b.concat(y), o) {
				var T = g, E;
				g = [];
				for (var x = 0; x < T.length; x++) T[x] != E && g.push(T[x]), E = T[x];
			}
...
var Pi = {
	...
	setOption(e, t) {
		switch (e) {
			case "keyMap":
				this.state.keyMap = t;
				break;
			case "textwidth": this.state.textwidth = t;
		}
	}
	getOption(e) {
		switch (e) {
	...
	getTokenTypeAt(e) {
		var t = this.indexFromPos(e), n = (fe(this.cm6.state, t)?.resolve(t))?.type?.name || "";
...
function $c(e, t) {
	return new Promise((n) => {
		...
		i.type = "text", i.className = "veditor-doc-picker-filter", i.placeholder = "Filter documents...";
		let a = document.createElement("ul");
		a.className = "veditor-doc-picker-list", r.appendChild(i), r.appendChild(a);
		let o = [...e], s = 0, c = [];
		function l(e, t) {
			let n = e.toLowerCase(), r = t.toLowerCase(), i = 0;
			for (let e = 0; e < n.length && i < r.length; e++) n[e] === r[i] && i++;
			return i === r.length;
		}
		function u(e) {
			return (e.bufferIndex == null ? "   " : `${e.bufferIndex}: `) + e.label;
		}
		function d() {
			a.innerHTML = "", c = [], o.forEach((e, t) => {
				let r = document.createElement("li");
				if (r.className = "veditor-doc-picker-item", t === s && r.classList.add("highlighted"), e.active && r.classList.add("active-buffer"), r.textContent = u(e), e.active) {
					let e = document.createElement("span");
					e.className = "veditor-doc-picker-badge", e.textContent = " %", r.appendChild(e);
				}
				r.addEventListener("click", () => {
					m(), n(e.id);
				}), a.appendChild(r), c.push(r);
			}), c[s]?.scrollIntoView({ block: "nearest" });
		}
		function f() {
			let t = i.value;
			o = t ? e.filter((e) => l(e.label, t)) : [...e], s = Math.min(s, Math.max(0, o.length - 1)), d();
		}
		function p(e) {
			let t = s;
			s = Math.max(0, Math.min(s + e, o.length - 1)), t !== s && (c[t]?.classList.remove("highlighted"), c[s]?.classList.add("highlighted"), c[s]?.scrollIntoView({ block: "nearest" }));
		}
		function m() {
			r.close(), r.remove();
		}
		i.addEventListener("input", () => {
			...
			let t = i.value === "";
			e.key === "ArrowDown" || e.key === "j" && t ? (e.preventDefault(), p(1)) : e.key === "ArrowUp" || e.key === "k" && t ? (e.preventDefault(), p(-1)) : e.key === "Enter" ? (e.preventDefault(), o.length > 0 && (m(), n(o[s].id))) : e.key === "Escape" && (e.preventDefault(), m(), n(null));
		}), r.addEventListener("click", (e) => {
			e.target === r && (m(), n(null));
		}), t.appendChild(r), r.showModal(), i.focus(), f();
	});
}
//#endregion
//#region src/veditor.ts
var el = "0.40.0", tl = /https?:\/\/[^\s)\]>]+/g;
function nl(e, t, n) {
	tl.lastIndex = 0;
	let r, i = null;
...
} }), Q = null, $ = "veditor", il = null, al = null, ol = null, sl = null, cl;
function ll() {
	return Bc()?.view ?? null;
}
function ul() {
...
async function ml() {
	let e = fl();
	if (!e) return;
	await e.onSave(), dl(Fl());
	let t = Bc();
	t && (t.dirtyNotified = !1), hl();
}
function hl() {
	if (!Q) return;
	let e = Il(ul());
...
async function gl(e) {
	...
	let t = Bc();
	t && (t.view.state.doc.toString() !== t.savedContent && (await t.callbacks.onSave(), t.savedContent = t.view.state.doc.toString(), t.dirtyNotified = !1), Xc(Q)), Uc(e), Zc(e, Q), hl();
	let n = Bc();
...
async function _l() {
	...
	let { content: o, label: s, callbacks: c } = await e.onLoadDocument(a), { view: l, compartments: u } = Al(o, c);
	Hc(a, s, l, o, c, u), await gl(a);
}
function vl(e) {
...
function Sl(e, t) {
	...
	n.className = "veditor-mode-toggle", n.type = "button", n.addEventListener("click", () => Hl()), e.appendChild(n), il = n, xl(t);
}
function Cl(e, t, n) {
	if (e) {
		n.onQuit();
...
function Dl(e, t, n) {
	...
	i("Save & Close", "veditor-cm-save", async () => {
		...
		let e = fl();
		e && Cl(!1, n, e);
	}), i("Cancel", "veditor-cm-cancel", () => {}), document.body.appendChild(r), Tl = r;
	...
	let s = (e) => {
		r.contains(e.target) || (El(), document.removeEventListener("pointerdown", s, !0), document.removeEventListener("keydown", c, !0));
	}, c = (e) => {
		e.key === "Escape" && (e.stopPropagation(), El(), document.removeEventListener("pointerdown", s, !0), document.removeEventListener("keydown", c, !0));
	};
	setTimeout(() => {
...
function kl(e) {
	...
	for (let e of t) {
		n += `<div class="veditor-help-section"><div class="veditor-help-title">${Ol(e.title)}</div>`;
		for (let [t, r] of e.entries) n += `<div class="veditor-help-row"><span class="veditor-help-key">${Ol(t)}</span><span class="veditor-help-desc">${Ol(r)}</span></div>`;
		n += "</div>";
	}
	let r = document.createElement("dialog");
	r.className = "veditor-help", r.innerHTML = n;
	let i = () => {
		r.close(), r.remove(), ll()?.focus();
	};
	r.addEventListener("cancel", (e) => {
		e.preventDefault(), i();
	}), r.addEventListener("keydown", (e) => {
		e.key === "q" || e.key === "Enter" ? (e.preventDefault(), i()) : e.key === "j" ? (e.preventDefault(), r.scrollBy(0, 40)) : e.key === "k" && (e.preventDefault(), r.scrollBy(0, -40));
	}), r.addEventListener("click", (e) => {
		e.target === r && i();
	}), document.body.appendChild(r), r.showModal();
}
function Al(e, t) {
	let n = $, r = cl?.clickableLinks ?? !0, i = cl?.autoSaveMs ?? 0, a = Sc(n), o = Q, s = {
		vim: new d(),
		cua: new d(),
		wrap: new d(),
		list: new d()
	...
	l.push(E.updateListener.of((e) => {
		if (e.docChanged) {
			...
			n && (r && !n.dirtyNotified ? (n.dirtyNotified = !0, n.callbacks.onDirty?.()) : r || (n.dirtyNotified = !1)), r && i > 0 && e.view.dom.parentNode && (sl !== null && clearTimeout(sl), sl = setTimeout(() => {
				ml();
			}, i));
		}
	})), r && l.push(rl), cl?.extensions && l.push(...cl.extensions);
	let u = w.create({
		doc: e,
		extensions: l
	}), f = new E({ state: u });
...
function Nl(e, t, n, r) {
	zl(), Q = e, cl = r, e.classList.add("veditor-dirty-aware"), e.classList.remove("veditor-dirty");
	let i = r?.storagePrefix ?? "veditor";
	$ = i;
	let a = Sc(i);
	if (Ml(), r?.exCommands) for (let [e, t] of Object.entries(r.exCommands)) J.defineEx(e, e, t);
	...
	let o = J.getRegisterController(), s = o.pushText.bind(o);
	o.pushText = (e, t, n, r, i) => {
		s(e, t, n, r, i), e !== "_" && navigator.clipboard.writeText(n).catch(() => {
			window.postMessage({
	...
	let { view: c, compartments: l } = Al(t, n);
	...
	e.addEventListener("keydown", (t) => {
		...
		Il(ul()) && (e.preventDefault(), e.returnValue = "");
	}, { signal: al.signal }), Sl(e, a);
	...
	return e.addEventListener("touchstart", (e) => {
		...
		m && t.touches.length === 0 && (m = !1, Dl(h, g, e));
...
function zl() {
	Qc(), il &&= (il.remove(), null), Q &&= (Q.classList.remove("veditor-dirty", "veditor-dirty-aware", "veditor-vim-normal", "veditor-vim-insert"), null), al &&= (al.abort(), null), ol &&= (ol.abort(), null), sl !== null && (clearTimeout(sl), sl = null), El(), cl = void 0;
}
function Bl() {
	let e = ll();
...
function Hl() {
	...
	Cc($, n);
	let r = Q ? bl(Q, $) : [];
	return e.dispatch({ effects: [t.vim.reconfigure(n ? da() : []), t.cua.reconfigure(n ? [] : r)] }), xl(n), n ? (Q?.classList.add("veditor-vim-normal"), Q?.classList.remove("veditor-vim-insert"), yl(e)) : Q?.classList.remove("veditor-vim-normal", "veditor-vim-insert"), e.focus(), n;
}
function Ul() {
	return Sc($);
}
...
function Yl(e) {
	try {
		let t = e.slice(-1e3);
		localStorage.setItem(ql, JSON.stringify(t));
	} catch {}
}
function Xl(e, t) {
	let n = {
...
function ru() {
	...
	i.readOnly = !0, i.value = tu(), i.style.cssText = "\n    flex: 1;\n    padding: 10px;\n    background: #1e1e1e;\n    color: #e0e0e0;\n    border: none;\n    font-family: monospace;\n    font-size: 12px;\n    resize: none;\n    overflow: auto;\n  ", i.scrollTop = i.scrollHeight;
	...
	return s.textContent = "Refresh", s.style.cssText = "\n    padding: 6px 12px;\n    background: #1976d2;\n    color: white;\n    border: none;\n    border-radius: 4px;\n    cursor: pointer;\n    font-size: 12px;\n  ", s.addEventListener("click", () => {
		i.value = tu(), i.scrollTop = i.scrollHeight;
...
var iu = "0.40.0";
//#endregion
export { iu as VERSION, nu as clearLogs, Nl as createEditor, ru as createLogViewer, Kl as createVimInput, zl as destroyEditor, Vl as executeExCommand, Bl as exitInsertMode, Rl as focusEditor, Oc as getAutoSaveMs, Fl as getEditorContent, tu as getFormattedLogs, bc as hashTarget, Pl as insertAtCursor, Il as isEditorDirty, Ul as isVimMode, eu as logDebug, Zl as logError, $l as logInfo, Ql as logWarn, Gl as requestQuit, Wl as requestSave, Ll as setEditorContent, Hl as toggleVimMode };
